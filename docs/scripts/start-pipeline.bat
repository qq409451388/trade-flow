...
REM ============================================================
set PIPELINE_JAR=C:\dev\pipeline\trade-pipeline-1.0.0-SNAPSHOT.jar
set JVM_OPTS=-Xms512m -Xmx1536m -XX:+UseG1GC -Dfile.encoding=UTF-8 -Duser.timezone=Asia/Shanghai
...
if not exist "%PIPELINE_JAR%" (
    echo [错误] Pipeline jar 不存在：%PIPELINE_JAR%
    pause
    exit /b 1
)
...
REM ============================================================
echo =============================================
echo  正在启动 Pipeline 服务...
echo  外部配置目录：%~dp0
echo =============================================
cd /d "%~dp0"
java %JVM_OPTS% -jar "%PIPELINE_JAR%"
