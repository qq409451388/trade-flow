...
Pipeline 默认每分钟按订单、支付通道分别自动拉取一批耗尽事件，每批100条；多实例通过
`pipeline_event_pull_control` 的 MySQL 租约互斥。单轮只处理一批，避免持续失败事件形成热循环，
下一轮继续从 Ingress 当前最旧的未ACK耗尽事件开始。人工主动拉取接口继续保留。
