...
不建设 Dead Letter Stream。PEL 只用于进程中断及 ACK 收尾恢复，不承担无限业务重试。
...
避免多实例并发恢复。Pipeline readiness 会检查业务库、Storage 数据源、Redis 和对应消费组。
...
- 富友支付文档只定义字符串 `paySsn`，没有独立消息版本；Ingress 当前要求它可转换为非负 `long`。
  如果真实流水号可能包含字母，需要确定第三方提供的稳定版本来源，不能用本地时间、event ID 或 Hash 代替。
